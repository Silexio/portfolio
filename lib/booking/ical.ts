...
function parseIcsDate(
  ...
  if (date) {
    const [, y, m, d] = date;
    return {
      ms: zonedWallToUtc(+y, +m, +d, 0, 0, fallbackTz).getTime(),
      allDay: true,
    };
  }
  ...
  if (zulu)
    return { ms: Date.UTC(+y, +m - 1, +d, +hh, +mm, +ss), allDay: false };
  const tz = prop.params.TZID || fallbackTz;
  return {
    ms: zonedWallToUtc(+y, +m, +d, +hh, +mm, tz).getTime(),
    allDay: false,
  };
}
...
function expand(
  ...
  const until = rule.UNTIL
    ? (parseIcsDate({ name: "UNTIL", params: {}, value: rule.UNTIL }, "UTC")
        ?.ms ?? Infinity)
    : Infinity;
  const byDay = rule.BYDAY
    ? rule.BYDAY.split(",").map((d) => d.trim().slice(-2).toUpperCase())
    : null;
  ...
  for (let i = 0; i < MAX_OCCURRENCES && emitted < count; i++) {
    ...
    const onCountedWeek =
      !byDay ||
      freq !== "WEEKLY" ||
      ((weekStart(ms) - anchorWeek) / (7 * DAY_MS)) % interval === 0;
    ...
    if (freq === "DAILY") cursor.setUTCDate(cursor.getUTCDate() + interval);
      ...
      cursor.setUTCDate(cursor.getUTCDate() + (byDay ? 1 : 7 * interval));
...
function eventToIntervals(
  ...
  const starts = rrule
    ? expand(start.ms, parseRrule(rrule.value), to)
    : [start.ms];
